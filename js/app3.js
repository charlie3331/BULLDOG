...
function agregarItemAlCarrito2(titulo, precio, imagenSrc) {
    console.log("Entré a la función");

    // Verificamos si el item ya existe en el carrito virtual
    for (var i = 0; i < carritoVirtual.length; i++) {
        if (carritoVirtual[i].titulo === titulo) {
            console.log(`Índice encontrado: i = ${i}`);
            // Incrementamos la cantidad en el carrito virtual
            carritoVirtual[i].cantidad += 1;
            
            // Actualizamos el total del carrito
            actualizarTotalCarrito2();
    ...
    // Si no existe, agregamos el nuevo item al carrito virtual
    carritoVirtual.push({
        titulo: titulo,
        precio: precio,
        imagenSrc: imagenSrc,
...
// Función para sincronizar el carrito virtual con el DOM
function sincronizarCarritoConDOM() {
    var itemsCarrito = document.querySelector('.carrito-items');

    // Verificar que el contenedor exista
    if (!itemsCarrito) {
        console.error('El contenedor del carrito no está disponible en el DOM.');
        return;
    }
    ...
    // Recorremos el carrito virtual y creamos los elementos en el DOM
    carritoVirtual.forEach(item => {
        ...
        div.querySelector('.btn-eliminar').addEventListener('click', () => eliminarItemCarrito2(item.titulo));

        // Agregamos el evento de restar cantidad
        div.querySelector('.restar-cantidad').addEventListener('click', () => restarCantidad2(item.titulo));

        // Agregamos el evento de sumar cantidad
        div.querySelector('.sumar-cantidad').addEventListener('click', () => sumarCantidad2(item.titulo));
...
function actualizarTotalCarrito2() {
    var total = carritoVirtual.reduce((suma, item) => suma + (item.precio * item.cantidad), 0);
    document.querySelector('.carrito-total').textContent = `Total: $${total.toFixed(2)}`;
}

// Función para eliminar un item del carrito
function eliminarItemCarrito2(titulo) {
    carritoVirtual = carritoVirtual.filter(item => item.titulo !== titulo);
    sincronizarCarritoConDOM();
    actualizarTotalCarrito();
}

// Función para restar cantidad de un item
function restarCantidad2(titulo) {
    carritoVirtual.forEach(item => {
...
// Función para sumar cantidad de un item
function sumarCantidad2(titulo) {
    carritoVirtual.forEach(item => {
...
// Mantener visible el botón de pagar y el campo de descuento
document.getElementById("boton-pagar").style.visibility = 'visible';
document.getElementById("input-descuento-container").style.visibility = 'visible';

// Asegúrate de que estos no se vean afectados por otros eventos
document.getElementById('carrito').addEventListener('click', (event) => {
    // Evitar que el carrito o cualquier otro contenedor afecte estos elementos
    document.getElementById("boton-pagar").style.visibility = 'visible';
    document.getElementById("input-descuento-container").style.visibility = 'visible';
});


function redirigirConCarrito() {
    ...
    for (let i = 0; i < carritoItems.length; i++) {
        ...
        carritoData.push({ titulo, cantidad, precio, imagen });
    }
